...
CLI.prototype.do_helen = function (subcmd, opts, args, callback) {
    ...
    p('Launching %d machine%s using the "%s" smrt profile:\n'
        + '    SDC_URL=%s\n'
        + '    SDC_ACCOUNT=%s\n'
        + '    SDC_KEY_ID=%s\n',
        opts.num, (opts.num === 1 ? '' : 's'),
        smrt.currProfileName, prof.url, prof.account, prof.key);
        ...
        async.series([
            function chooseName(next) {
                var now = (new Date()).toISOString().replace(
                    /[-:]/g, '').replace(/\.\d+/, '');
                name = smrt.randomNameSync('greeks') + '-' + now;
                next();
            },
            ...
            function createMachine(next) {
                ...
                smrt.createMachine(machOpts, function (err, res) {
                    ...
                    p('Creating machine "%s" (%s %s, %s): id %s',
                        name, img.name, img.version, pkg.name, res.id);
                    machine.res = res;
                    next();
                });
            },
