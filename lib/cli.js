...
CLI.prototype.do_paris = function (subcmd, opts, args, callback) {
    ...
    var prof = this.smrt.currProfile;
    p('Current profile "%s": %s (account=%s, keyId=%s)', prof.name, prof.url,
...
CLI.prototype.do_helen.help = (
    ...
    + '\n'
    + 'Usage:\n'
    + '     {{name}} helen [-n <num>] [<image>[:<package>] ...]\n'
    + '\n'
    + '{{options}}'
    + '\n'
            ...
            smrt.listHomericMachines(function (err, machines) {
                ...
                async.eachLimit(machines, 2,
                    function delMachine(machine, nextMachine) {
                        ...
                        smrt.deleteMachine(machine.id, nextMachine);
                    },
                    next);
            });
        },
...
CLI.prototype.do_aphrodite = function (subcmd, opts, args, callback) {
    ...
    async.series([
        ...
        function coalProfile(next) {
            ...
            smrt.createOrUpdateProfile(prof, {setCurrent: false}, next);
        }
    ], callback)
};
...
CLI.prototype.do_iris = function (subcmd, opts, args, callback) {
    ...
    async.parallel([
        ...
        if (opts.json) {
            ...
            p('# %d Homeric machine%s (profile: %s)\n',
                hMachines.length, (hMachines.length === 1 ? '' : 's'),
                smrt.currProfileName);
            if (hMachines.length === 0) {
                p('(none)');
            } else {
                var columns = 'shortId,name,state,img,package,primaryIp';
                if (opts.long) {
                ...
                common.tabulate(hMachines, {
                    columns: columns,
                    sort: 'created',
                    validFields: 'shortId,id,name,type,state,image,img,memory,disk,created,updated,primaryIp,firewall_enabled,package'
                });
            }
            p('\n# %d Homeric image%s (profile: %s)\n',
            ...
            if (hImages.length === 0) {
                ...
                common.tabulate(hImages, {
                    columns: columns,
                    sort: 'published_at',
                    validFields: 'shortId,id,name,version,type,state,os,published_at,public'
                });
            }
        }
        callback();
...
CLI.prototype.do_trojan = function (subcmd, opts, args, callback) {
    ...
    async.series([
            ...
            function tryOnce() {
                ...
                exec(cmd, function (err, stdout, stderr) {
                    ...
                    if (!err) {
                        ...
                        {
                            p('SSH to proto still failing after 1 minute, will '
        ...
        function customize(next) {
            ...
            // login... it includes motd.tail, so do that too.
            var cmd = protoSsh + " 'echo This was smrtified. >>/etc/motd; echo This was smrtified. >>/etc/motd.tail'";
            //e('customize cmd:', cmd)
            var execOpts = {
                timeout: 30000,
                maxBuffer: 1024 * 1024
        ...
        //XXX TODO: pull these two funcs up to the upper async.series
        async.series([
            ...
            function helen(next) {
                ...
                var argv = opts.helen.map(function (h) { return img.id + ':' + h; });
                if (opts.json) {
                    argv.push('--json');
                }
                ...
                //XXX was this:
                //self.do_helen('helen', {json: opts.json}, args, next);
            }
