                ...
                function gather(m, next2) {
                    ...
                    smrt.randomImage(m.imgTerm, function (imgErr, img) {
                        if (imgErr)
                            return next2(imgErr);
                        m.img = img;
                        //XXX fake image to test cloudapi response to a bogus uuid
                        //m.img = img = {id: m.imgTerm, name: 'foo', version: '1.2.3', os: 'smartos'};
        ...
        async.series([
            ...
            function waitForProvision(next) {
                ...
                smrt.waitForMachineProvision(waitOpts, function (err, finalMach) {
                    ...
                    bar.log(format('Created machine "%s" (%s %s, %s): id %s',
                        machine.name, machine.img.name, machine.img.version,
                        machine.pkg.name, machine.id));
                    next();
                });
            },
            // TODO maybe
...
CLI.prototype.do_aphrodite = function (subcmd, opts, args, callback) {
    ...
    async.series([
        function setupCoal(next) {
            ...
            // TODO How to handle not having a key?
        ...
        function setDate(next) {
            ...
            var cmd = SSH + " root@10.99.99.7 'svcadm clear ntp; date " + currTime + "'";
            exec(cmd, function (err, stdout, stderr) {
                if (err) {
    ...
    async.each(
        ...
        function doneGathering(err) {
            ...
            for (j = 0; j < hImages.length; j++) {
                ...
                if (i.origin) { // Need PUBAPI-720 first for this.
                    i.orig = format('%s/%s',
                        imageFromIdFromProf[i.profile][i.origin].name,
                        imageFromIdFromProf[i.profile][i.origin].version);
                } else {
                    i.orig = null;
                }
            }
            if (opts.json) {
                ...
                if (hMachines.length === 0) {
                    ...
                    // TODO want long id by default for now. Can switch to
                    // shortId when all the other cmds supports expanding that.
                ...
                if (hImages.length === 0) {
                    ...
                    // TODO want long id by default for now. Can switch to
                    // shortId when all the other cmds supports expanding that.
                    var columns = 'id,name,version,state,os,public,orig';
                    if (opts.long) {
                        columns = 'id,name,version,state,os,public,orig';
                    }
                    if (opts.all) {
                        columns = 'profile,' + columns
                    }
                    common.tabulate(hImages, {
                        ...
                        validFields: 'profile,shortId,id,name,version,type,state,os,published_at,public,orig'
                    });
                }
            }
            callback();
...
CLI.prototype.do_hermes = function (subcmd, opts, args, callback) {
    ...
    var results;   // Array of {machine: <machine>} for each running machine.
    var ssh;
    ...
    async.series([
        function getRunningMachines(next) {
            smrt.listHomericMachines(function (listErr, machines) {
                if (listErr)
                    return next(listErr);
                results = machines
                    .filter(function (m) { return m.state === 'running'; })
                    .map(function (m) { return {machine: m}; });
                next();
            });
        },
        function doPingCheck(next) {
            if (cmd) return next();
            async.each(
                results,
                function pingMachine(result, nextMachine) {
                    var ip = result.machine.primaryIp;
        ...
        function doSshCheck(next) {
            if (cmd) return next();
            async.each(
                results,
                function sshToMachine(result, nextMachine) {
                    // TODO: ubuntu for cert images
                    var sshCmd = ssh + ' root@' + result.machine.primaryIp + ' "echo hi"';
                    var execOpts = {
                        timeout: 30000
                    };
                    log.debug({sshCmd: sshCmd}, 'sshToMachine');
                    exec(sshCmd, execOpts, function (err, stdout, stderr) {
                        log.debug({sshCmd: sshCmd, err: err, stdout: stdout, stderr: stderr},
        ...
        function runSshCmd(next) {
            ...
            async.each(
                ...
                function sshToMachine(result, nextMachine) {
                    // TODO: ubuntu for cert images
                    // TODO: escaping 'cmd'
                    var sshCmd = format('%s root@%s "%s"', ssh,
                        result.machine.primaryIp, cmd);
                    var execOpts = {
                        timeout: 30000
                    };
                    log.debug({sshCmd: sshCmd}, 'sshToMachine');
        ...
        if (opts.json) {
            ...
            for (var i = 0; i < results.length; i++) {
                var r = results[i];
                var o = r.result.stdout.trimRight();
                if (r.result.stderr.trimRight()) {
                    if (o) {
                        o += '\n'
                    }
                    o += r.result.stderr.trimRight();
                }
                if (/\n/.test(o)) {
                    singleLine = false;
                }
            ...
            if (singleLine && !opts.long) {
                common.tabulate(rows, {
                    columns: 'id,code,output',
                    sort: 'id',
                    validFields: 'id,name,code,output'
                });
            } else {
                for (var i = 0; i < rows.length; i++) {
                    var r = rows[i];
                    p('=== Output from %s (%s): exit code %d ===',
                        r.id, r.name, r.code);
                    p(r.output);
                }
            }
            ...
            for (var i = 0; i < results.length; i++) {
                var r = results[i];
                r.machine.ping = r.pingCheck.success;
                r.machine.ssh = r.sshCheck.success;
                runningMachines.push(r.machine);
            }
...
CLI.prototype.do_hermes.options = [
    ...
    },
    {
        names: ['long', 'l'],
        type: 'bool',
        help: 'Force long output (not in a table) if output is all single-line.'
    }
];
CLI.prototype.do_hermes.help = (
    'Ping-check and ssh-check all running Homeric machines.\n'
    + 'An optional command can be given to run on all machines.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} hermes [<cmd>]\n'
    + '\n'
...
CLI.prototype.do_trojan = function (subcmd, opts, args, callback) {
    ...
    async.series([
            ...
            function tryOnce() {
                ...
                exec(cmd, execOpts, function (err, stdout, stderr) {
                    ...
                    if (!err) {
                        ...
                        next();
                    // XXX EASTONE-111 bites us in beta4 too. Go to 15 minutes.
                    } else if (end - start > 15 * 60 * 1000 /* 15 minutes */) {
                        if (!log.debug()) process.stderr.write('\n');
                        next(new Error('SSH to VM still does not work after 15 minutes'));
                    } else {
                        if (!log.debug()) process.stderr.write('.');
                        if (!loggedOneError &&
                            end - start > 1 * 60 * 1000 /* 1 minute */)
                        {
                            e('SSH to proto still failing after 1 minute, will '
                                + 'keep trying for 14 more minutes: %s (stderr=%s)',
                                err, stderr);
                            loggedOneError = true;
                        }
        ...
        function createImage(next) {
            var name = smrt.randomNameSync('trojans').replace(/ /g, '-');
            var createOpts = {
                machine: proto.id,
                name: name,
                //TODO: description
        ...
        function waitForImageCreate(next) {
            var waitOpts = {};
            if (opts.timeout) {
                waitOpts = opts.timeout * 1000;
            }
            smrt.waitForImageCreate(img.id, waitOpts, function (err, finalImg) {
                if (err) {
                    return next(err);
                    ...
                    return next(new Error(format(
                        ...
                        finalImg.state,
                        JSON.stringify(finalImg, null, 4))));
                }
                img = finalImg;
                next();
            });
...
CLI.prototype.do_trojan.options = [
    ...
    },
    {
        names: ['timeout', 't'],
        type: 'positiveInteger',
        help: 'Time (in seconds) to wait for image creation completion. '
            + 'Default is to not timeout.'
    },
    {
        names: ['old'],
        type: 'bool',
        help: 'Use the old process where we need to manually prepare the image and stop it.'
    }
];
...
CLI.prototype.do_delete_image = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    if (args.length !== 1) {
        return callback(new Error('incorrect number of args'));
    }
    var smrt = self.smrt;
    var uuid = args[0];

    smrt.deleteImage(uuid, function (err) {
        if (err) {
            if (err.statusCode === 410 && err.message[0] === '{') {
                // Stupid 410 Gone response from cloudapi gives the whole
                // deleted image JSON as the error "message".
                err.image = err.message;
                err.message = format('image %s is already deleted', uuid);
            }
            return callback(err);
        }
        p('Deleted image %s', uuid)
        callback();
    });
};
CLI.prototype.do_delete_image.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_delete_image.help = (
    'Delete the given image.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} delete-image <uuid>\n'
    + '\n'
    + '{{options}}'
);
...
CLI.prototype.do_image = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.getImage(args[0], function (err, image) {
        if (err)
            return callback(err);
        p(JSON.stringify(image, null, 4));
        callback();
    });
};
CLI.prototype.do_image.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
        ...
        if (opts.json) {
            ...
            if (images.length === 0) {
                ...
                common.tabulate(images, {
                    columns: 'id,name,version,state,flags,os,published',
                    sort: opts.sort || 'published_at',
                    validFields: 'id,name,version,type,state,os,published_at,public,flags,size,published,published_date'
                });
            }
        }
        callback();
...
CLI.prototype.do_machine = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.getMachine(args[0], function (err, machine) {
        if (err)
            return callback(err);
        p(JSON.stringify(machine, null, 4));
        callback();
    });
};
CLI.prototype.do_machine.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_machine_metadata = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.getMachineMetadata(args[0], function (err, metadata) {
        if (err)
            return callback(err);
        p(JSON.stringify(metadata, null, 4));
        callback();
    });
};
CLI.prototype.do_machine_metadata.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_machine_metadata.help = (
    'Show the given machine\'s metadata.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} machine-metadata <uuid>\n'
    + '\n'
    + '{{options}}'
);


CLI.prototype.do_update_machine_metadata = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    process.stdin.on("end", function() {
        var metadata = JSON.parse(s);
        smrt.updateMachineMetadata(id, metadata, function (err) {
            if (err)
                return callback(err);
            p('Machine %s metadata updated.', id)
            callback();
        });
    });

};
CLI.prototype.do_update_machine_metadata.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_update_machine_metadata.help = (
    'Update/replace the given machine\'s metadata.\n'
    + '\n'
    + 'Usage:\n'
    + '     ...metadata JSON... | {{name}} update-machine-metadata <uuid>\n'
    + '\n'
    + '{{options}}'
);
...
CLI.prototype.do_delete_machine = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.deleteMachine(uuid, function (err) {
        if (err) {
            if (err.statusCode === 410 && err.message[0] === '{') {
                // Stupid 410 Gone response from cloudapi gives the whole
                // deleted image JSON as the error "message".
                err.machine = err.message;
                err.message = format('machine %s is already deleted', uuid);
            }
            return callback(err);
        }
        p('Deleted machine %s', uuid)
        callback();
    });
};
CLI.prototype.do_delete_machine.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_delete_machine.help = (
    'Delete the given machine.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} delete-machine <uuid>\n'
    + '\n'
    + '{{options}}'
);

CLI.prototype.do_snapshots = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.listMachineSnapshots(args[0], function (err, snapshots) {
        if (err)
            return callback(err);
        if (opts.json) {
            p(JSON.stringify(snapshots, null, 4));
        } else if (snapshots.length) {
            XXX
            // TODO: add shortId, img
            common.tabulate(snapshots, {
                columns: 'id,name,state,package,primaryIp',
                sort: 'created',
                validFields: 'id,name,type,state,image,memory,disk,created,updated,primaryIp,firewall_enabled,package'
            });
...
CLI.prototype.do_snapshots.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
CLI.prototype.do_snapshots.help = (
    'List snapshots for a given machine.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} snapshots <machine-id>\n'
    + '\n'
    + '{{options}}'
);

CLI.prototype.do_audit = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    smrt.getMachineAudit(args[0], function (err, audit) {
        if (err)
            return callback(err);
        if (opts.json) {
            ...
            audit.forEach(function (a) {
                a.ip = a.caller.ip;
                a.keyId = a.caller.keyId;
                a.from = format("ip %s key %s", a.caller.ip, a.caller.keyId);
            });
            common.tabulate(audit, {
                columns: opts.o,
                sort: 'time',
                validFields: 'time,action,success,from,ip,keyId'
            });
        }
        callback();
    });
};
CLI.prototype.do_audit.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
    ...
    },
    {
        names: ['o'],
        type: 'string',
        help: 'Columns (comma-separated) to show. Default: "time,action,success,ip".',
        default: 'time,action,success,ip'
    }
];
CLI.prototype.do_audit.help = (
    'List audit records for the given machine.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} audit <machine-id>\n'
    + '\n'
    + '{{options}}'
    + 'Column "from" is a special stringified version of the "caller" object.'
);
...
CLI.prototype.do_reboot = function (subcmd, opts, args, callback) {
    ...
    async.each(
        args,
        function rebootMach(machineId, next) {
            // TODO: support abbreviated IDs
            p('Reboot machine %s', machineId);
            smrt.rebootMachine(machineId, next);
        },
        callback);
    // TODO: option to wait until all restarted
};
...
CLI.prototype.do_reboot.help = (
    'Reboot the given machine(s)\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} reboot <machine-id>...\n'
    + '\n'
    + '{{options}}'
);


CLI.prototype.do_ssh = function (subcmd, opts, args, callback) {
    var self = this;
    if (opts.help) {
        this.do_help('help', {}, [subcmd], callback);
        return;
    ...
    var image;
    async.series([
        function getMach(next) {
            smrt.getMachine(id, function (getErr, machine_) {
                if (getErr)
                    return next(getErr);
                machine = machine_;
                next();
        ...
        function getImg(next) {
            smrt.getImage(machine.image, function (getErr, image_) {
                if (getErr)
                    return next(getErr);
                image = image_;
                next();
        ...
        function sshToIt(next) {
            var user = 'root';
            // TODO: support custom images of ubuntu-certified images.
            if (image && ~image.name.indexOf('ubuntu-certified')) {
                user = 'ubuntu';
            }
            argv.push(user + '@' + machine.primaryIp);
            sshArgs.forEach(function (a) { argv.push(a); });
            var spawnOpts = {
                stdio: 'inherit'
            };
            log.debug({argv: argv, spawnOpts: spawnOpts}, 'sshToMachine');
            var ssh = spawn(argv[0], argv.slice(1), spawnOpts);
            //ssh.stdout.pipe(process.stdout);
            //ssh.stderr.pipe(process.stderr);
            // TODO: only `next` on exit *and* close
            ssh.on('exit', function(exitStatus) {
                if (exitStatus) {
                    next(new Error('ssh errored out: ' + exitStatus));
                } else {
                    next();
                }
            });
