 ...
 *
 * @param term {String} A image id, name or name substring.
...
SMRT.prototype.randomPackage = function randomPackage(img, term, callback) {
...
SMRT.prototype.createImageFromMachine = function createImageFromMachine(
    ...
    this.cloudapi.createImageFromMachine(createOpts, function (err, img) {
        if (err) {
            return callback(err);
        }
        callback(null, img);
